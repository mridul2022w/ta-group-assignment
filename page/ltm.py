...
def call_ltm(uber_review):
    ...
    st.write("Computing Beta Matrix")
    # invoke func
    beta_df = build_beta_df(lda_model=lda_model, id2word=id2word)
    
    st.write("Computing Gamma Matrix")
    # now apply func
    gamma_df = build_gamma_df(lda_model=lda_model, corpus0=uber_review_text,id2word=id2word)
    ...
    limit1=19
    step1=1
    st.write("Coherence score chart")
    # Show graph
    ...
    st.write("Taking Optimal Topic")
    optimal_model = model_list[Optimal_numTopics]
    model_topics = optimal_model.show_topics(formatted=False)


    # Get main topic in each document
    st.write("Choosing main topic in each document")
    gamma_df = build_gamma_df(lda_model=optimal_model, corpus0=uber_review_text,id2word=id2word)
